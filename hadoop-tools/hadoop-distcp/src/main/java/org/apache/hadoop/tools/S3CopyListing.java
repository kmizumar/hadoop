...
/**
 * The S3CopyListing is responsible for making the exhaustive list of
 * all files/directories under its specified list of input-paths.
 * These are written into the specified copy-listing file.
 * Note: The S3CopyListing doesn't handle wild-cards in the input-paths.
 */
public class S3CopyListing extends CopyListing {
  public static final Logger LOG =
      LoggerFactory.getLogger(S3CopyListing.class);
   ...
   */
  protected S3CopyListing(Configuration configuration, Credentials credentials) {
  ...
  @VisibleForTesting
  protected S3CopyListing(Configuration configuration,
  ...
  private void writeToFileListing(SequenceFile.Writer fileListWriter,
    ...
    if (fileStatus.isDirectory() || !shouldCopy(fileStatus.getPath())) {
      if (LOG.isDebugEnabled()) {
        LOG.debug("skipping {}", fileStatus.getPath());
      }
      return;
    }
    ...
    // no directory should come here since destination fs schema is s3a://
